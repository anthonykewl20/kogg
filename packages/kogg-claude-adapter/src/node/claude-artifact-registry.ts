...
export class ClaudeArtifactRegistry implements BackendApplicationContribution {
  private started: Promise<void> | undefined; private value: ClaudeReleaseProjection = blocked(); private approvalValue: ClaudeCommercialUseApprovalV1 | undefined;
  ...
  private async verifyApproval(): Promise<void> { this.approvalValue = undefined; try { await trustedDirectory(this.assetsRoot); const approval = await bounded(path.join(this.assetsRoot, 'claude-commercial-use-approval-v1.json'), 16_384); const keyBytes = await bounded(path.join(this.assetsRoot, 'claude-commercial-approval-public-key.pem'), 16_384); const record = parse(approval, this.now()); const unsigned = Object.fromEntries(FIELDS.filter(field => field !== 'signature').map(field => [field, record[field]])); let valid = false; try { const key = createPublicKey(keyBytes); valid = key.asymmetricKeyType === 'ed25519' && verify(null, Buffer.from(canonical(unsigned)), key, decodeSignature(record.signature)); } catch { /* observability-exempt: outer legal.verify.failure classifies invalid signature or key material without echoing it. */ valid = false; } if (!valid) throw new Error('invalid'); this.approvalValue = Object.freeze({ ...record }); this.value = { ...blocked(), legalApproved: true, safeCode: 'CLAUDE_ARTIFACT_MISMATCH' }; claudeLog('legal.verify.success', { signingKeyId: record.signingKeyId }); } catch { /* observability-exempt: legal.verify.failure is the sanitized terminal event for absent or invalid approval input. */ this.approvalValue = undefined; this.value = blocked(); claudeLog('legal.verify.failure', { safeCode: 'CLAUDE_LEGAL_APPROVAL_REQUIRED' }); } }
}
