...
export class ClaudeProcessHostGate {
  ...
  private async cleanupOnce(): Promise<void> {
    ...
    if (this.identity) { try { await within(this.authority.signal(this.identity, 'SIGTERM'), this.attempt.deadlines.gracefulExitMs + this.attempt.deadlines.terminateMs, 'CLAUDE_RESIDUAL_PROCESS'); claudeLog('process.signal.completed', { attemptId: this.attempt.attemptId, signalClass: 'terminate' }); let inventory = await this.inventory(); if (inventory.memberCount > 0) { await within(this.authority.signal(this.identity, 'SIGKILL'), this.attempt.deadlines.killMs, 'CLAUDE_RESIDUAL_PROCESS'); claudeLog('process.signal.completed', { attemptId: this.attempt.attemptId, signalClass: 'kill' }); inventory = await this.inventory(); } if (!inventory.identityVerified || inventory.memberCount !== 0) failure = new ClaudeProcessFault('CLAUDE_RESIDUAL_PROCESS'); } catch { // observability-exempt: Owner errors and process identities are discarded behind the aggregate cleanup failure.
        failure = new ClaudeProcessFault('CLAUDE_RESIDUAL_PROCESS'); } }
    if (!failure && this.process) { try { if (wasRunning) this.process.exited('signal'); this.process.cleanup(); } catch { // observability-exempt: The logical registry error contains private state; external cleanup already completed and the closed aggregate failure is emitted below.
        failure = new ClaudeProcessFault('CLAUDE_RESIDUAL_PROCESS'); } }
    this.phaseValue = failure ? 'faulted' : 'cleaned'; if (failure) { this.markLogicalFailed('PROCESS_SIGNALLED'); claudeLog('process.cleanup.failed', { attemptId: this.attempt.attemptId, safeCode: failure.code }); throw failure; } claudeLog('process.cleanup.completed', { attemptId: this.attempt.attemptId, residualCount: 0 });
  }
  private async failAndClean(code: ClaudeProcessFault['code']): Promise<never> { this.phaseValue = 'faulted'; this.markLogicalFailed('PROCESS_SPAWN_FAILED'); claudeLog('process.failure', { attemptId: this.attempt.attemptId, safeCode: code }); try { await this.cleanup(); } catch { // observability-exempt: Cleanup emitted the overriding closed residual failure.
  ...
  private fault(code: ClaudeProcessFault['code']): never { this.phaseValue = 'faulted'; this.markLogicalFailed('PROCESS_SPAWN_FAILED'); claudeLog('process.failure', { attemptId: this.attempt.attemptId, safeCode: code }); throw new ClaudeProcessFault(code); }
  private markLogicalFailed(code: 'PROCESS_SIGNALLED' | 'PROCESS_SPAWN_FAILED'): void { try { this.process?.failed(code, 'ClaudeProcessFault'); } catch { // observability-exempt: The closed adapter failure remains authoritative when the logical registry also refuses its failure transition.
      return; } }
}
