...
test('reports unfinished evidence capabilities explicitly instead of inferring health from the bridge', async () => {
  const checks = await new KernelDiagnosticContributor(bridge(async () => ({ status: 'degraded', journal: 'missing', capabilities: CAPABILITIES })), owner(), owner()).diagnose();
...
test('fails all kernel diagnostics safely when the bridge throws', async () => {
  const checks = await new KernelDiagnosticContributor(bridge(async () => { throw new Error('content-bearing canary'); }), owner(), owner()).diagnose();
  assert.equal(checks.length, 13);
...
function owner(): never { return { diagnostics: () => ({ integrity: true, sourceEventCount: 0, projectedEventCount: 0 }) } as never; }
