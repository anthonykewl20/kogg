...
const DESCRIPTOR_FIELDS = new Set(['schemaVersion', 'adapterKey', 'adapterVersion', 'protocolId', 'protocolVersion', 'providerIds', 'capabilityIds', 'executionKind', 'cancellation', 'usageModes', 'ownerKind', 'enabled']);
const EXECUTION_KINDS = new Set(['in-process', 'supervised-host', 'ranex-bridge']); const USAGE_MODES = new Set(['provider-cumulative', 'provider-delta', 'kogg-derived']); const OWNER_KINDS = new Set(['kogg', 'ranex', 'theia']);
...
function validateDescriptor(descriptor: AdapterDescriptorV1): void { if (Object.keys(descriptor).some(key => !DESCRIPTOR_FIELDS.has(key)) || descriptor.schemaVersion !== '1' || descriptor.cancellation !== 'cooperative-and-owned-cleanup' || !EXECUTION_KINDS.has(descriptor.executionKind) || !OWNER_KINDS.has(descriptor.ownerKind) || typeof descriptor.enabled !== 'boolean') throw new Error('ADAPTER_DESCRIPTOR_INVALID'); if (!Array.isArray(descriptor.providerIds) || !Array.isArray(descriptor.capabilityIds) || !Array.isArray(descriptor.usageModes) || descriptor.usageModes.some(mode => !USAGE_MODES.has(mode))) throw new Error('ADAPTER_DESCRIPTOR_INVALID'); [descriptor.adapterKey, descriptor.protocolId, ...descriptor.providerIds, ...descriptor.capabilityIds].forEach(symbolic); semver(descriptor.adapterVersion); semver(descriptor.protocolVersion); if (!descriptor.providerIds.length || descriptor.providerIds.length > 32 || descriptor.capabilityIds.length > 64 || descriptor.usageModes.length > 3 || new Set(descriptor.providerIds).size !== descriptor.providerIds.length || new Set(descriptor.capabilityIds).size !== descriptor.capabilityIds.length || new Set(descriptor.usageModes).size !== descriptor.usageModes.length) throw new Error('ADAPTER_DESCRIPTOR_INVALID'); }
