...
function parseObservation(line: string): AdapterObservationV1 { if (Buffer.byteLength(line) > 4096) throw new FixtureAdapterError('ADAPTER_OBSERVATION_INVALID'); const value = JSON.parse(line) as Partial<AdapterObservationV1>; if (!value.sequence || !['ready', 'provider-request-started', 'provider-request-settled', 'activity', 'usage', 'completed', 'failed'].includes(value.kind ?? '')) throw new FixtureAdapterError('ADAPTER_OBSERVATION_INVALID'); return value as AdapterObservationV1; }
