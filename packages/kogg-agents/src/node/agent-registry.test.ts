...
test('runs a real supervised fixture host through completion and proves cleanup', async () => {
  ...
  try {
    ...
    assert.equal(terminal.terminalCode, 'AGENT_OK'); assert.equal(terminal.ownedResourceCount, '0'); assert.equal(terminal.deadlinePolicyId, 'interactive-v1');
    assert.deepEqual(new Set(terminal.deadlines.map(deadline => deadline.deadlineClass)), new Set(['absolute', 'cleanup', 'first-activity', 'handshake', 'idle', 'provider-request'])); assert.equal(terminal.deadlines.every(deadline => deadline.generation === '1' && deadline.active === false && Number.isFinite(Date.parse(deadline.expiresAt))), true);
    assert.deepEqual(terminal.usage, { status: 'complete', source: 'provider-cumulative', inputTokens: '1', outputTokens: '1', totalTokens: '2' });
...
test('persists and binds a validated governed workspace grant for a mutating attempt', async () => {
  ...
  const worktreeId = '41000000-0000-4000-8000-000000000001'; const requests: Parameters<AgentWorkspaceAuthority['prepareWorkspace']>[0][] = []; const activations: Parameters<AgentWorkspaceAuthority['activateWorkspace']>[0][] = []; const finalizations: Parameters<AgentWorkspaceAuthority['finalizeWorkspace']>[0][] = []; const authority: AgentWorkspaceAuthority = {
    async prepareWorkspace(request) { requests.push(request); return { allowed: true, code: 'AGENT_OK', worktreeId, workspaceGrantDigest: 'b'.repeat(64) }; },
    async activateWorkspace(request) { activations.push(request); return { completed: true, code: 'AGENT_OK' }; },
    async finalizeWorkspace(request) { finalizations.push(request); return { completed: true, code: 'AGENT_OK' }; }
  };
  ...
  try {
    ...
    const terminal = await poll(() => registry.getAttempt(result.attempt!.attemptId), value => value.state === 'cleaned'); assert.equal(activations.length, 1); assert.equal(finalizations.length, 1); assert.equal(finalizations[0]?.outcome, 'completed'); assert.equal(activations[0]?.workspaceGrantDigest, 'b'.repeat(64));
    await registry.onStop(); restarted = new AgentRegistry({ resolveAdmission: async () => ADMISSION }, new TestOperations(), new AdapterRegistry(), new LocalCredentialLeaseAuthority(), MODE_AUTHORITY); await restarted.onStart(); const projected = await restarted.getAttempt(result.attempt!.attemptId); assert.equal(projected.worktreeId, worktreeId); assert.equal(projected.deadlinePolicyId, 'interactive-v1'); assert.deepEqual(projected.deadlines, terminal.deadlines);
...
test('migrates legacy terminal attempts to explicit unknown mode authority without inventing trust', async () => {
  ...
  try {
    ...
    database.exec('ALTER TABLE attempts DROP COLUMN deadline_policy_id; ALTER TABLE attempts DROP COLUMN authority_operation; ALTER TABLE attempts DROP COLUMN authority_sequence; ALTER TABLE attempts DROP COLUMN authority_mode;'); database.close();
    ...
    assert.equal(migrated.authorityMode, 'unknown'); assert.equal(migrated.authoritySequence, '0'); assert.equal(migrated.authorityOperation, 'unknown'); assert.equal(migrated.deadlinePolicyId, 'legacy-v1');
...
test('refuses startup when a durable deadline policy binding is corrupted', async () => {
  const directory = await mkdtemp(path.join(os.tmpdir(), 'kogg-agents-deadline-integrity-')); const prior = process.env.KOGG_STATE_DIR; process.env.KOGG_STATE_DIR = directory;
  const first = new AgentRegistry({ resolveAdmission: async () => ADMISSION }, new TestOperations(), new AdapterRegistry(), new LocalCredentialLeaseAuthority(), MODE_AUTHORITY);
  try {
    await first.onStart(); const role = await first.createRoleRevision(roleRequest('20000000-0000-4000-8000-000000000025', '0')); assert.ok(role.role);
    const refused = await first.startAttempt({ schemaVersion: '1', requestId: '30000000-0000-4000-8000-000000000025', expectedRegistryRevision: role.registryRevision, taskAdmissionId: ADMISSION.taskAdmissionId, roleRevisionId: role.role.roleRevisionId, providerId: 'kogg.fixture', modelId: 'fixture.echo', adapterKey: 'missing.adapter', adapterVersion: '1.0.0', deadlinePolicyId: 'interactive-v1' }); assert.ok(refused.attempt); await first.onStop();
    const database = new DatabaseSync(path.join(directory, 'agents', 'registry.sqlite3')); database.prepare('UPDATE attempts SET deadline_policy_id=? WHERE attempt_id=?').run('INVALID POLICY', refused.attempt.attemptId); database.close();
    await assert.rejects(new AgentRegistry({ resolveAdmission: async () => ADMISSION }, new TestOperations(), new AdapterRegistry(), new LocalCredentialLeaseAuthority(), MODE_AUTHORITY).onStart(), /AGENT_REGISTRY_INTEGRITY_FAILED/u);
  } finally { await first.onStop(); if (prior === undefined) delete process.env.KOGG_STATE_DIR; else process.env.KOGG_STATE_DIR = prior; await rm(directory, { recursive: true, force: true }); }
});
