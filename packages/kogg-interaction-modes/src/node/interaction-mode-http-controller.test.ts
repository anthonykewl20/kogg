...
test('admits transition confirmation only through authenticated same-origin CSRF-protected HTTP', { timeout: 20_000 }, async context => {
  ...
  const controller = new InteractionModeHttpController(browserAuth, transitionAuthority, registry, new ModeTransitionCoordinator(registry, [])); const app = express(); browserAuth.configure(app); controller.configure(app);
  ...
  const configuration = { schemaVersion: 1, kind: 'build', roleRevisionId: '80000000-0000-4000-8000-000000000009', providerId: 'fixture', modelId: 'fixture.echo', adapterKey: 'kogg.fixture', adapterVersion: '1.0.0', deadlinePolicyId: 'standard', targetId: 'qualified-linux' } as const;
  const request = { transitionId: '80000000-0000-4000-8000-000000000001', requestId: '80000000-0000-4000-8000-000000000002', taskId: TASK.taskId, expectedSequence: '0', fromMode: 'plan', toMode: 'build', requestedConfigurationDigest: digest(configuration) };
  ...
  const pending = accepted.json() as { state: string; safeCode: string; challengeDigest: string; mode: { state: string } }; assert.equal(pending.state, 'awaiting-confirmation'); assert.equal(pending.safeCode, 'MODE_EXPANSION_CONFIRMATION_REQUIRED'); assert.equal(pending.mode.state, 'transition-pending');
  const confirm = { requestId: '80000000-0000-4000-8000-000000000003', transitionId: request.transitionId, taskId: TASK.taskId, challengeDigest: pending.challengeDigest, explicitGesture: true, configuration };
  const refusedConfirm = await post(`${base}/kogg/modes/transitions/confirm`, confirm, { cookie, origin: base, csrf: 'wrong' }); assert.equal(refusedConfirm.status, 403);
  const confirmed = await post(`${base}/kogg/modes/transitions/confirm`, confirm, { cookie, origin: base, csrf }); assert.equal(confirmed.status, 200);
  const result = confirmed.json() as { state: string; safeCode: string; mode: { selectedMode: string } }; assert.equal(result.state, 'quarantined'); assert.equal(result.safeCode, 'MODE_CLEANUP_FAILED'); assert.equal(result.mode.selectedMode, 'plan');
});
