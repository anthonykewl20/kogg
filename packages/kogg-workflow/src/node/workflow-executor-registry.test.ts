...
test('exact-attests supervised agent dispatch while refusing forged artifacts, digest tampering, and incomplete joins', () => {
  ...
  assert.throws(() => registry.resolveExact('control.join', { ...binding, artifactDigest: 'f'.repeat(64) }), /WORKFLOW_EXECUTOR_INCOMPATIBLE/u); const approvalBinding = registry.binding('approval.specification'); assert.ok(approvalBinding); assert.equal(registry.resolveExact('approval.specification', approvalBinding).executionKind, 'task-admission-witness'); const agentBinding = registry.binding('implementation.agent'); assert.ok(agentBinding); assert.equal(registry.resolveExact('implementation.agent', agentBinding).executionKind, 'supervised-registry'); assert.equal(registry.binding('approval.continue'), undefined); assert.equal(registry.binding('tool.build'), undefined);
