...
import type { AgentBindingAuthorizer, AgentSafeCode, AttemptProjectionV1, KoggAgentsService, StartAttemptRequestV1 } from '@kogg/agents/lib/common/agents-protocol';
...
test('binds every closed node kind to one canonical catalog digest while refusing unavailable external executors', () => {
  ...
  assert.equal(diagnostics.valid, true); assert.equal(diagnostics.entryCount, 14); assert.equal(diagnostics.availableExecutorCount, 10); assert.equal(diagnostics.unavailableExecutorCount, 4);
  assert.match(catalog.digest, /^[0-9a-f]{64}$/u); assert.deepEqual(catalog.entries.filter(entry => entry.executor.status === 'available').map(entry => entry.kind), ['approval.specification','control.condition','control.finally','control.group','control.join','control.parallel','implementation.agent','probe.agent','pseudocode.agent','research.agent']);
...
test('durably dispatches exact workflow agent bindings through the supervised agent registry', async () => {
  const root = await mkdtemp(path.join(os.tmpdir(), 'kogg-workflow-agent-run-')); const database = path.join(root, 'workflow.sqlite3'); const compiler = workflowCompiler(); const fixture = agentDispatcher();
  try {
    const registry = new WorkflowRegistry(compiler, MODE_AUTHORITY, AGENT_BINDING_AUTHORITY, database); registry.setTaskAdmissionAuthority(TASK_ADMISSION_AUTHORITY); registry.setAgentAttemptDispatcher(fixture.dispatcher); await registry.onStart(); const graph = agentGraph();
    const saved = await registry.saveVersion({ requestId: '2a000000-0000-4000-8000-000000000001', templateId: '1a000000-0000-4000-8000-000000000001', expectedVersionNumber: 0, graph }); if (saved.kind !== 'completed' || !saved.version) throw new Error('Expected immutable agent workflow');
    const compiled = await registry.compile({ requestId: '2a000000-0000-4000-8000-000000000002', versionId: saved.version.versionId }); if (compiled.kind !== 'completed' || !compiled.plan) throw new Error('Expected compiled agent workflow');
    const taskAdmissionId = '2a000000-0000-4000-8000-000000000003'; const completed = await registry.admitRun({ requestId: '2a000000-0000-4000-8000-000000000004', planId: compiled.plan.planId, taskAdmissionId });
    assert.equal(completed.kind, 'completed'); assert.equal(completed.run?.runId, taskAdmissionId); assert.equal(completed.run?.completedNodeCount, 1); assert.equal(fixture.starts.length, 1); const configuration = graph.nodes[0]!.configuration!; assert.deepEqual(fixture.starts[0], { schemaVersion: '1', requestId: fixture.starts[0]!.requestId, expectedRegistryRevision: '7', taskAdmissionId, roleRevisionId: configuration.roleRevisionId, providerId: configuration.providerId, modelId: configuration.modelId, adapterKey: configuration.adapterKey, adapterVersion: configuration.adapterVersion, deadlinePolicyId: configuration.deadlinePolicyId });
    const store = new DatabaseSync(database); const attempt = store.prepare('SELECT external_attempt_id,state,safe_code FROM workflow_node_attempts').get() as { external_attempt_id: string; state: string; safe_code: string }; assert.deepEqual({ ...attempt }, { external_attempt_id: fixture.attemptId, state: 'succeeded', safe_code: 'WORKFLOW_OK' }); assert.equal((store.prepare("SELECT count(*) AS count FROM workflow_outbox WHERE operation_kind='research.agent' AND phase='completed'").get() as { count: number }).count, 1); store.close(); assert.deepEqual(await registry.admitRun({ requestId: '2a000000-0000-4000-8000-000000000004', planId: compiled.plan.planId, taskAdmissionId }), completed); assert.equal(fixture.starts.length, 1); await registry.onStop();
  } finally { await rm(root, { recursive: true, force: true }); }
});

test('maps supervised agent refusal and residual cleanup to closed workflow failure state', async () => {
  const root = await mkdtemp(path.join(os.tmpdir(), 'kogg-workflow-agent-failure-')); const database = path.join(root, 'workflow.sqlite3'); const compiler = workflowCompiler(); const fixture = agentDispatcher('CLEANUP_FAILED', 'cleanup_failed');
  try {
    const registry = new WorkflowRegistry(compiler, MODE_AUTHORITY, AGENT_BINDING_AUTHORITY, database); registry.setTaskAdmissionAuthority(TASK_ADMISSION_AUTHORITY); registry.setAgentAttemptDispatcher(fixture.dispatcher); await registry.onStart();
    const saved = await registry.saveVersion({ requestId: '2b000000-0000-4000-8000-000000000001', templateId: '1b000000-0000-4000-8000-000000000001', expectedVersionNumber: 0, graph: agentGraph() }); if (saved.kind !== 'completed' || !saved.version) throw new Error('Expected immutable failing agent workflow'); const compiled = await registry.compile({ requestId: '2b000000-0000-4000-8000-000000000002', versionId: saved.version.versionId }); if (compiled.kind !== 'completed' || !compiled.plan) throw new Error('Expected compiled failing agent workflow');
    const failed = await registry.admitRun({ requestId: '2b000000-0000-4000-8000-000000000003', planId: compiled.plan.planId, taskAdmissionId: '2b000000-0000-4000-8000-000000000004' }); assert.equal(failed.kind, 'failed'); assert.equal(failed.code, 'WORKFLOW_CLEANUP_FAILED'); assert.equal(failed.run?.failedNodeCount, 1); const store = new DatabaseSync(database); assert.equal((store.prepare('SELECT state FROM workflow_node_attempts').get() as { state: string }).state, 'failed'); assert.equal((store.prepare('SELECT residual_process_count FROM workflow_runs').get() as { residual_process_count: number }).residual_process_count, 1); store.close(); assert.equal((await registry.diagnostics()).residualProcessCount, 1); await registry.onStop();
  } finally { await rm(root, { recursive: true, force: true }); }
});
...
test('executes specification approval only from the exact current task-admission witness', async () => {
  for (const mutateAtExecution of [false, true]) {
    const root = await mkdtemp(path.join(os.tmpdir(), `kogg-workflow-spec-approval-${mutateAtExecution}-`)); const database = path.join(root, 'workflow.sqlite3'); const compiler = workflowCompiler(); let resolutions = 0;
    const authority: TaskAdmissionAuthority = { async resolveAdmission(taskAdmissionId) { resolutions++; const admission = taskAdmission(taskAdmissionId); return mutateAtExecution && resolutions === 2 ? { ...admission, approvalId: '2e000000-0000-4000-8000-000000000099' } : admission; } };
    try {
      const registry = new WorkflowRegistry(compiler, MODE_AUTHORITY, AGENT_BINDING_AUTHORITY, database); registry.setTaskAdmissionAuthority(authority); await registry.onStart(); const saved = await registry.saveVersion({ requestId: mutateAtExecution ? '2f000000-0000-4000-8000-000000000001' : '2e000000-0000-4000-8000-000000000001', templateId: mutateAtExecution ? '1f000000-0000-4000-8000-000000000001' : '1e000000-0000-4000-8000-000000000001', expectedVersionNumber: 0, graph: approvalGraph() }); if (saved.kind !== 'completed' || !saved.version) throw new Error('Expected immutable specification approval workflow'); const compiled = await registry.compile({ requestId: mutateAtExecution ? '2f000000-0000-4000-8000-000000000002' : '2e000000-0000-4000-8000-000000000002', versionId: saved.version.versionId }); if (compiled.kind !== 'completed' || !compiled.plan) throw new Error('Expected compiled specification approval workflow');
      const result = await registry.admitRun({ requestId: mutateAtExecution ? '2f000000-0000-4000-8000-000000000003' : '2e000000-0000-4000-8000-000000000003', planId: compiled.plan.planId, taskAdmissionId: mutateAtExecution ? '2f000000-0000-4000-8000-000000000004' : '2e000000-0000-4000-8000-000000000004' }); assert.equal(resolutions, 2); assert.equal(result.kind, mutateAtExecution ? 'failed' : 'completed'); assert.equal(result.code, mutateAtExecution ? 'WORKFLOW_AUTHORITY_EXPANSION' : 'WORKFLOW_OK'); const store = new DatabaseSync(database); assert.equal((store.prepare('SELECT state FROM workflow_node_attempts').get() as { state: string }).state, mutateAtExecution ? 'failed' : 'succeeded'); store.close(); await registry.onStop();
    } finally { await rm(root, { recursive: true, force: true }); }
  }
});
...
test('cancels overdue supervised agents and refuses unverified residual processes', async () => {
  for (const cancelTerminates of [true, false]) {
    const root = await mkdtemp(path.join(os.tmpdir(), `kogg-workflow-agent-timeout-${cancelTerminates}-`)); const database = path.join(root, 'workflow.sqlite3'); const compiler = workflowCompiler(); const fixture = agentDispatcher('AGENT_OK', 'cleaned', { activeUntilCancel: true, cancelTerminates });
    try {
      const registry = new WorkflowRegistry(compiler, MODE_AUTHORITY, AGENT_BINDING_AUTHORITY, database); registry.setTaskAdmissionAuthority(TASK_ADMISSION_AUTHORITY); registry.setAgentAttemptDispatcher(fixture.dispatcher); await registry.onStart();
      const saved = await registry.saveVersion({ requestId: cancelTerminates ? '2c000000-0000-4000-8000-000000000001' : '2d000000-0000-4000-8000-000000000001', templateId: cancelTerminates ? '1c000000-0000-4000-8000-000000000001' : '1d000000-0000-4000-8000-000000000001', expectedVersionNumber: 0, graph: agentGraph(1_000) }); if (saved.kind !== 'completed' || !saved.version) throw new Error('Expected immutable overdue agent workflow'); const compiled = await registry.compile({ requestId: cancelTerminates ? '2c000000-0000-4000-8000-000000000002' : '2d000000-0000-4000-8000-000000000002', versionId: saved.version.versionId }); if (compiled.kind !== 'completed' || !compiled.plan) throw new Error('Expected compiled overdue agent workflow');
      const failed = await registry.admitRun({ requestId: cancelTerminates ? '2c000000-0000-4000-8000-000000000003' : '2d000000-0000-4000-8000-000000000003', planId: compiled.plan.planId, taskAdmissionId: cancelTerminates ? '2c000000-0000-4000-8000-000000000004' : '2d000000-0000-4000-8000-000000000004' }); assert.equal(failed.kind, 'failed'); assert.equal(failed.code, cancelTerminates ? 'WORKFLOW_DEADLINE' : 'WORKFLOW_RESIDUAL_PROCESS'); assert.equal(fixture.cancels.length, cancelTerminates ? 1 : 2); assert.equal((await registry.diagnostics()).residualProcessCount, cancelTerminates ? 0 : 1); await registry.onStop();
    } finally { await rm(root, { recursive: true, force: true }); }
  }
});
...
function agentGraph(absoluteDeadlineMs = 60_000): EditableWorkflowGraphV1 & { nodes: EditableWorkflowNodeV1[] } { const agent = node('3a000000-0000-4000-8000-000000000001', 'research.agent', ['read-repository','invoke-provider']); const configuration = { ...agent.configuration!, absoluteDeadlineMs }; return { schemaVersion: '1', projectId: PROJECT, nodes: [{ ...agent, configuration, configurationDigest: workflowDigest('node-configuration', configuration) }], edges: [] }; }
function approvalGraph(): EditableWorkflowGraphV1 & { nodes: EditableWorkflowNodeV1[] } { return { schemaVersion: '1', projectId: PROJECT, nodes: [node('3e000000-0000-4000-8000-000000000001', 'approval.specification', ['record-approval'])], edges: [] }; }
function controlGraph(condition: 'always' | 'prior-success', single = false): EditableWorkflowGraphV1 { const configuration = { schemaVersion: '1' as const, absoluteDeadlineMs: 1_000, target: 'project-read-only' as const, condition }; const first: EditableWorkflowNodeV1 = { nodeId: '34000000-0000-4000-8000-000000000001', kind: condition === 'always' ? 'control.group' : 'control.condition', kindVersion: '1', configuration, configurationDigest: workflowDigest('node-configuration', configuration), requestedEffects: [], retry: { maxAttempts: 1, backoffMs: 0, sideEffectPolicy: 'none' } }; if (single) return { schemaVersion: '1', projectId: PROJECT, nodes: [first], edges: [] }; const second = node('34000000-0000-4000-8000-000000000002', 'control.finally', []); return { schemaVersion: '1', projectId: PROJECT, nodes: [first, second], edges: [edge('44000000-0000-4000-8000-000000000001', first.nodeId, second.nodeId)] }; }
function branchingControlGraph(): EditableWorkflowGraphV1 { const configuration = { schemaVersion: '1' as const, absoluteDeadlineMs: 1_000, target: 'project-read-only' as const, condition: 'prior-success' as const }; const start = node('36000000-0000-4000-8000-000000000001', 'control.group', []); const condition: EditableWorkflowNodeV1 = { nodeId: '36000000-0000-4000-8000-000000000002', kind: 'control.condition', kindVersion: '1', configuration, configurationDigest: workflowDigest('node-configuration', configuration), requestedEffects: [], retry: { maxAttempts: 1, backoffMs: 0, sideEffectPolicy: 'none' } }; const selected = node('36000000-0000-4000-8000-000000000003', 'control.group', []); const skipped = node('36000000-0000-4000-8000-000000000004', 'control.group', []); const join = node('36000000-0000-4000-8000-000000000005', 'control.join', []); const cleanup = node('36000000-0000-4000-8000-000000000006', 'control.finally', []); return { schemaVersion: '1', projectId: PROJECT, nodes: [start, condition, selected, skipped, join, cleanup], edges: [edge('46000000-0000-4000-8000-000000000001', start.nodeId, condition.nodeId), edge('46000000-0000-4000-8000-000000000002', condition.nodeId, selected.nodeId, 'true'), edge('46000000-0000-4000-8000-000000000003', condition.nodeId, skipped.nodeId, 'false'), edge('46000000-0000-4000-8000-000000000004', selected.nodeId, join.nodeId), edge('46000000-0000-4000-8000-000000000005', skipped.nodeId, join.nodeId), edge('46000000-0000-4000-8000-000000000006', join.nodeId, cleanup.nodeId, 'finally')] }; }
...
function agentDispatcher(terminalCode: AgentSafeCode = 'AGENT_OK', state: AttemptProjectionV1['state'] = 'cleaned', options: { activeUntilCancel?: boolean; cancelTerminates?: boolean } = {}) {
  const starts: StartAttemptRequestV1[] = []; const cancels: unknown[] = []; const attemptId = '3a000000-0000-4000-8000-000000000002'; let cancelled = false;
  const attempt = (): AttemptProjectionV1 => { const active = options.activeUntilCancel && !cancelled; const projectedState = active ? 'active' : state; return { schemaVersion: '1', attemptId, rootAttemptId: attemptId, attemptRevision: active ? '8' : '9', registryRevision: '8', taskId: TASK, projectId: PROJECT, repositoryId: REPOSITORY, specificationId: '18000000-0000-4000-8000-000000000001', approvalId: '18000000-0000-4000-8000-000000000002', runId: starts.at(-1)?.taskAdmissionId, roleRevisionId: agentGraph().nodes[0]!.configuration!.roleRevisionId!, adapterKey: 'kogg.fixture', adapterVersion: '1.0.0', providerId: 'kogg.fixture', requestedModelId: 'fixture.research', observedModelId: 'fixture.research', state: projectedState, terminalCode: active ? undefined : terminalCode, activityCount: '1', childCount: '0', ownedResourceCount: projectedState === 'cleanup_failed' ? '1' : '0', usage: { status: 'complete', source: 'provider-cumulative', totalTokens: '1' }, requestedAt: '2026-08-27T00:00:00.000Z', stateChangedAt: '2026-08-27T00:00:01.000Z' }; };
  const dispatcher = { async snapshot() { return { schemaVersion: '1', registryRevision: '7', admission: 'enabled', roles: [], attempts: [], adapters: [] } as const; }, async startAttempt(request: StartAttemptRequestV1) { starts.push(request); return { kind: 'completed', code: 'AGENT_OK', registryRevision: '8', attempt: attempt() } as const; }, async getAttempt() { return attempt(); }, async cancelAttempt(request: unknown) { cancels.push(request); if (options.cancelTerminates !== false) cancelled = true; return options.cancelTerminates === false ? { kind: 'conflict', code: 'REGISTRY_REVISION_CONFLICT', registryRevision: '8' } as const : { kind: 'completed', code: terminalCode, registryRevision: '8', attempt: attempt() } as const; } } as unknown as KoggAgentsService;
