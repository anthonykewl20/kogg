...
test('versions a validated graph immutably and compiles the policy-owned trust spine across restart', async () => {
  const root = await mkdtemp(path.join(os.tmpdir(), 'kogg-workflow-registry-')); const database = path.join(root, 'workflow.sqlite3'); const compiler = new WorkflowCompiler();
  try {
    ...
    const diagnostics = await restarted.diagnostics(); assert.equal(diagnostics.canonicalMismatchCount, 0); assert.equal(diagnostics.planMismatchCount, 0); assert.equal(diagnostics.residualProcessCount, 0); await restarted.onStop();
...
test('fails closed on forged anchors, cycles, ambiguous joins, widened authority, retries, and stale version writes', async () => {
  const compiler = new WorkflowCompiler(); const forged = structuredClone(validGraph()) as unknown as { nodes: Array<Record<string, unknown>> }; forged.nodes[0]!.kind = 'anchor.controlled-merge'; assert.equal(compiler.validate(forged).code, 'WORKFLOW_ANCHOR_BYPASS');
...
test('refuses startup after immutable graph corruption and diagnostics fail as a complete catalog', async () => {
  const root = await mkdtemp(path.join(os.tmpdir(), 'kogg-workflow-integrity-')); const database = path.join(root, 'workflow.sqlite3'); const compiler = new WorkflowCompiler();
  try {
