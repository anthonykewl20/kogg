...
test('binds every closed node kind to one canonical catalog digest while refusing unavailable external executors', () => {
  ...
  assert.equal(diagnostics.valid, true); assert.equal(diagnostics.entryCount, 14); assert.equal(diagnostics.availableExecutorCount, 9); assert.equal(diagnostics.unavailableExecutorCount, 5);
  assert.match(catalog.digest, /^[0-9a-f]{64}$/u); assert.deepEqual(catalog.entries.filter(entry => entry.executor.status === 'available').map(entry => entry.kind), ['control.condition','control.finally','control.group','control.join','control.parallel','implementation.agent','probe.agent','pseudocode.agent','research.agent']);
