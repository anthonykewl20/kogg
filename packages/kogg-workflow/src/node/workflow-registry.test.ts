...
test('versions a validated graph immutably and compiles the policy-owned trust spine across restart', async () => {
  ...
  try {
    const registry = new WorkflowRegistry(compiler, MODE_AUTHORITY, database); await registry.onStart(); const graph = validGraph();
...
test('refuses startup after immutable graph corruption and diagnostics fail as a complete catalog', async () => {
  ...
  try {
    const registry = new WorkflowRegistry(compiler, MODE_AUTHORITY, database); await registry.onStart(); await registry.saveVersion({ requestId: '20000000-0000-4000-8000-000000000020', templateId: TEMPLATE, expectedVersionNumber: 0, graph: validGraph() }); await registry.onStop();
    ...
    const restarted = new WorkflowRegistry(compiler, MODE_AUTHORITY, database); await assert.rejects(restarted.onStart(), /WORKFLOW_STORE_INTEGRITY/u);
...
test('refuses startup when an immutable template catalog binding is changed', async () => {
  ...
  try {
    const registry = new WorkflowRegistry(compiler, MODE_AUTHORITY, database); await registry.onStart(); await registry.saveVersion({ requestId: '20000000-0000-4000-8000-000000000030', templateId: TEMPLATE, expectedVersionNumber: 0, graph: validGraph() }); await registry.onStop();
    ...
    await assert.rejects(new WorkflowRegistry(compiler, MODE_AUTHORITY, database).onStart(), /WORKFLOW_STORE_INTEGRITY/u);
  } finally { await rm(root, { recursive: true, force: true }); }
});
...
test('run admission validates the immutable plan and refuses unavailable executors before any run state exists', async () => {
  ...
  try {
    const registry = new WorkflowRegistry(compiler, MODE_AUTHORITY, database); await registry.onStart();
...
test('run admission refuses before executor selection when task mode authority or project binding is unavailable', async () => {
  const root = await mkdtemp(path.join(os.tmpdir(), 'kogg-workflow-mode-authority-')); const database = path.join(root, 'workflow.sqlite3'); const compiler = workflowCompiler(); let calls = 0;
  ...
  try {
    const registry = new WorkflowRegistry(compiler, refusedAuthority, database); await registry.onStart();
    ...
    assert.deepEqual(unavailable, { kind: 'refused', code: 'WORKFLOW_AUTHORITY_EXPANSION' }); assert.equal(calls, 2); assert.equal((await registry.diagnostics()).recoveryBacklogCount, 0); await registry.onStop();
  } finally { await rm(root, { recursive: true, force: true }); }
});
...
test('restart quarantines durable scheduler and outbox ambiguity without replaying dispatch', async () => {
  ...
  try {
    const first = new WorkflowRegistry(compiler, MODE_AUTHORITY, database); await first.onStart();
    ...
    const recovered = new WorkflowRegistry(compiler, MODE_AUTHORITY, database); await recovered.onStart(); const diagnostics = await recovered.diagnostics();
...
test('publishes immutable scheduler recovery facts through a stable workflow owner across restart', async () => {
  ...
  try {
    ...
    const recovered = new WorkflowRegistry(compiler, MODE_AUTHORITY, database); recovered.setOwnerSink(projection); await recovered.onStart();
    ...
    const restarted = new WorkflowRegistry(compiler, MODE_AUTHORITY, database); restarted.setOwnerSink(projection); await restarted.onStart(); assert.equal(projection.diagnostics().acceptedEventCount, 1);
...
test('refuses startup after stable workflow owner identity corruption', async () => {
  ...
  try {
    const first = new WorkflowRegistry(compiler, MODE_AUTHORITY, database); await first.onStart(); await first.onStop();
...
test('refuses startup after immutable scheduler recovery fact corruption', async () => {
  ...
  try {
    const first = new WorkflowRegistry(compiler, MODE_AUTHORITY, database); await first.onStart(); const saved = await first.saveVersion({ requestId: '20000000-0000-4000-8000-000000000070', templateId: TEMPLATE, expectedVersionNumber: 0, graph: validGraph() }); if (saved.kind !== 'completed' || !saved.version) throw new Error('Expected immutable workflow version'); const compiled = await first.compile({ requestId: '20000000-0000-4000-8000-000000000071', versionId: saved.version.versionId }); if (compiled.kind !== 'completed' || !compiled.plan) throw new Error('Expected compiled workflow plan'); await first.onStop();
    ...
    const recovered = new WorkflowRegistry(compiler, MODE_AUTHORITY, database); await recovered.onStart(); await recovered.onStop(); const corrupt = new DatabaseSync(database); corrupt.exec('DROP TRIGGER workflow_immutable_scheduler_events_update'); corrupt.prepare("UPDATE workflow_scheduler_events SET safe_code='WORKFLOW_OK'").run(); corrupt.close();
    await assert.rejects(new WorkflowRegistry(compiler, MODE_AUTHORITY, database).onStart(), /WORKFLOW_STORE_INTEGRITY/u);
  } finally { await rm(root, { recursive: true, force: true }); }
});
