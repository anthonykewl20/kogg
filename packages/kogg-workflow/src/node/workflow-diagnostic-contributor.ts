  ...
  async diagnose(): Promise<readonly KoggDiagnosticCheck[]> {
    try {
      ...
      return [
        ...
        { id: 'workflow.authority', status: 'fail', summary: 'Task-scoped mode entry is integrated; role, provider, evidence, verdict, and merge grants remain unavailable.', details: { safeCode: 'WORKFLOW_AUTHORITY_EXPANSION' } },
        ...
        { id: 'workflow.accessibility', status: 'pass', summary: 'The spatial canvas and structured outline expose the same add, remove, reorder, validate, version, and compile operations.', details: { editorViews: 2, sharedSemanticGraph: true } },
        { id: 'workflow.source-maps', status: value.sourceMapsPresent ? 'pass' : 'fail', summary: value.sourceMapsPresent ? 'Workflow browser editor, backend compiler, and registry source maps are available.' : 'Workflow browser or backend source maps are unavailable.' }
      ];
    } catch (error) {
