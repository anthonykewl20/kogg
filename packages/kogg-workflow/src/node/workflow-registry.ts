...
export class WorkflowRegistry implements KoggWorkflowService, BackendApplicationContribution {
  ...
  private async executeRun(runId: string, planId: string, graph: EditableWorkflowGraphV1, taskAdmissionId: string): Promise<WorkflowMutationResult> {
    ...
    while (pending.size) {
      ...
      const output = 'output' in result ? result.output : undefined; const handledFailure = result.kind !== 'completed' && result.residualProcessCount === 0 && graph.edges.some(edge => edge.sourceNodeId === node.nodeId && ['failure','finally'].includes(edge.sourcePort)); const durableOutput = handledFailure ? 'failure' : output; this.schedulerTransaction(() => { this.db().prepare("UPDATE workflow_outbox SET phase='completed',safe_code=?,updated_at=? WHERE outbox_id=? AND phase='claimed'").run(result.code, finishedAt, outboxId); this.db().prepare("UPDATE workflow_node_attempts SET state=?,safe_code=?,output=?,updated_at=? WHERE run_id=? AND node_id=? AND attempt=1 AND state='running'").run(result.kind === 'completed' ? 'succeeded' : 'failed', result.code, durableOutput ?? '', finishedAt, runId, node.nodeId); });
