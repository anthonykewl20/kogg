...
export type WorkflowExternalAnchorKind = 'anchor.evidence-admitted' | 'anchor.ranex-pass-current' | 'anchor.merge-preflight' | 'anchor.controlled-merge';
...
const EXTERNAL_ANCHORS: readonly WorkflowExternalAnchorKind[] = ['anchor.evidence-admitted','anchor.ranex-pass-current','anchor.merge-preflight','anchor.controlled-merge'];
const ANCHOR_IDS: Readonly<Record<WorkflowExternalAnchorKind, string>> = { 'anchor.evidence-admitted': '71000000-0000-4000-8000-000000000001', 'anchor.ranex-pass-current': '71000000-0000-4000-8000-000000000002', 'anchor.merge-preflight': '71000000-0000-4000-8000-000000000003', 'anchor.controlled-merge': '71000000-0000-4000-8000-000000000004' };
...
export class WorkflowRegistry implements KoggWorkflowService, BackendApplicationContribution {
  ...
  private async executeTrustSpine(runId: string, planId: string, completedNodeCount: number, skippedNodeCount: number, failedNodeCount: number): Promise<WorkflowMutationResult> {
    ...
    let subjectStateDigest = [...checkSubjects][0]!; let priorFactDigest = workflowDigest('trust-anchor', { anchor: 'anchor.checks-complete', checks: checks.map(row => ({ nodeId: text(row, 'node_id'), factDigest: text(row, 'fact_digest'), ownerSequence: text(row, 'owner_sequence') })) }); let lastOwnerSequence = 0;
    for (let index = 0; index < EXTERNAL_ANCHORS.length; index++) {
      const anchor = EXTERNAL_ANCHORS[index]!; const anchorId = ANCHOR_IDS[anchor]; const request: WorkflowTrustAnchorRequestV1 = Object.freeze({ runId, anchor, planDigest: text(run, 'plan_digest'), trustSpineDigest: text(run, 'trust_spine_digest'), taskAdmissionId: text(run, 'task_admission_id'), taskAdmissionDigest: text(run, 'task_admission_digest'), repositoryId: text(run, 'repository_id'), subjectStateDigest, priorFactDigest }); const requestDigest = workflowDigest('trust-anchor', request); const outboxId = randomUUID(); const startedAt = new Date().toISOString();
      this.schedulerTransaction(() => { this.db().prepare("INSERT INTO workflow_anchor_attempts(run_id,anchor_id,anchor_kind,ordinal,state,request_digest,input_subject_digest,input_fact_digest,fencing_token,safe_code,updated_at) VALUES(?,?,?,?,'running',?,?,?,?,'WORKFLOW_OK',?)").run(runId, anchorId, anchor, index + 1, requestDigest, subjectStateDigest, priorFactDigest, this.schedulerFencingToken, startedAt); this.db().prepare("INSERT INTO workflow_outbox(outbox_id,run_id,node_id,operation_kind,request_digest,fencing_token,phase,safe_code,created_at,updated_at) VALUES(?,?,?,?,?,?,'claimed','WORKFLOW_OK',?,?)").run(outboxId, runId, anchorId, anchor, requestDigest, this.schedulerFencingToken, startedAt, startedAt); });
      workflowLog('anchor.execution.started', { runId, anchor, ordinal: index + 1 }); let result: WorkflowTrustAnchorResultV1;
      try { result = await executeTrustAnchor(authority, request); if (!validTrustResult(result, subjectStateDigest, anchor, lastOwnerSequence)) throw new Error('invalid trust anchor result'); }
      ...
      if (result.kind !== 'completed') { workflowLog('anchor.execution.refused', { runId, anchor, ordinal: index + 1, safeCode: result.code, residualProcessCount: result.residualProcessCount }); return this.finishControlRun(runId, planId, 'failed', result.code, completedNodeCount, skippedNodeCount, failedNodeCount + 1, result.residualProcessCount); }
      workflowLog('anchor.execution.completed', { runId, anchor, ordinal: index + 1, safeCode: 'WORKFLOW_OK' }); subjectStateDigest = result.subjectStateDigest; priorFactDigest = result.factDigest; lastOwnerSequence = Number(result.ownerSequence);
    }
    return this.finishControlRun(runId, planId, 'completed', 'WORKFLOW_OK', completedNodeCount, skippedNodeCount, failedNodeCount);
  }
  ...
  private schedulerRecordsValid(): boolean {
    ...
    const malformedAnchors = this.countInvalid("SELECT count(*) AS count FROM workflow_anchor_attempts WHERE NOT ((anchor_kind='anchor.evidence-admitted' AND anchor_id='71000000-0000-4000-8000-000000000001' AND ordinal=1) OR (anchor_kind='anchor.ranex-pass-current' AND anchor_id='71000000-0000-4000-8000-000000000002' AND ordinal=2) OR (anchor_kind='anchor.merge-preflight' AND anchor_id='71000000-0000-4000-8000-000000000003' AND ordinal=3) OR (anchor_kind='anchor.controlled-merge' AND anchor_id='71000000-0000-4000-8000-000000000004' AND ordinal=4)) OR length(request_digest)<>64 OR request_digest GLOB '*[^0-9a-f]*' OR length(input_subject_digest)<>64 OR input_subject_digest GLOB '*[^0-9a-f]*' OR length(input_fact_digest)<>64 OR input_fact_digest GLOB '*[^0-9a-f]*' OR NOT ((state='succeeded' AND safe_code='WORKFLOW_OK' AND residual_process_count=0 AND length(result_subject_digest)=64 AND result_subject_digest NOT GLOB '*[^0-9a-f]*' AND length(result_fact_digest)=64 AND result_fact_digest NOT GLOB '*[^0-9a-f]*' AND owner_sequence GLOB '[1-9]*' AND owner_sequence NOT GLOB '*[^0-9]*' AND length(owner_sequence)<=16) OR (state<>'succeeded' AND result_subject_digest='' AND result_fact_digest='' AND owner_sequence=''))");
  ...
  private anchorRecordsValid(): boolean {
    ...
    for (const rows of grouped.values()) {
      ...
      for (const row of rows) {
        ...
        if (text(row, 'state') === 'succeeded') { const sequence = Number(text(row, 'owner_sequence')); if (!Number.isSafeInteger(sequence) || sequence <= lastOwnerSequence || (text(row, 'anchor_kind') !== 'anchor.controlled-merge' && text(row, 'result_subject_digest') !== text(row, 'input_subject_digest'))) return false; lastOwnerSequence = sequence; }
        prior = row;
      }
    }
    for (const row of this.db().prepare("SELECT r.run_id,v.graph_json FROM workflow_runs r JOIN compiled_plans p ON p.plan_id=r.plan_id JOIN template_versions v ON v.version_id=p.version_id WHERE r.state='completed'").all() as Row[]) { const graph = this.compiler.decodeAndValidate(JSON.parse(text(row, 'graph_json')) as unknown); if (graph.nodes.some(node => node.kind === 'check.deterministic')) { const anchors = grouped.get(text(row, 'run_id')) ?? []; if (anchors.length !== EXTERNAL_ANCHORS.length || anchors.some(anchor => text(anchor, 'state') !== 'succeeded')) return false; } }
    return true;
  }
