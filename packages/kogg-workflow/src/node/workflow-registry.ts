...
export class WorkflowRegistry implements KoggWorkflowService, BackendApplicationContribution {
  ...
  private async executeRun(runId: string, planId: string, graph: EditableWorkflowGraphV1, taskAdmissionId: string): Promise<WorkflowMutationResult> {
    ...
    while (pending.size) {
      ...
      if (node.kind === 'approval.continue' && !this.continuationReceipt(runId, node.nodeId)) return this.pauseForContinuation(runId, planId, node.nodeId, completedNodeCount, skippedNodeCount);
      ...
      const output = 'output' in result ? result.output : undefined; this.schedulerTransaction(() => { this.db().prepare("UPDATE workflow_outbox SET phase='completed',safe_code=?,updated_at=? WHERE outbox_id=? AND phase='claimed'").run(result.code, finishedAt, outboxId); this.db().prepare("UPDATE workflow_node_attempts SET state=?,safe_code=?,output=?,updated_at=? WHERE run_id=? AND node_id=? AND attempt=1 AND state='running'").run(result.kind === 'completed' ? 'succeeded' : 'failed', result.code, output ?? '', finishedAt, runId, node.nodeId); });
      if (result.kind !== 'completed' || !output) return this.finishControlRun(runId, planId, 'failed', result.code, completedNodeCount, skippedNodeCount, 1, result.residualProcessCount);
  ...
  private schedulerRecordsValid(): boolean {
    ...
    const malformedOutbox = number(this.db().prepare("SELECT count(*) AS count FROM workflow_outbox WHERE length(request_digest)<>64 OR request_digest GLOB '*[^0-9a-f]*'").get() as Row, 'count'); const malformedRouting = number(this.db().prepare("SELECT count(*) AS count FROM workflow_node_attempts WHERE routing_state NOT IN ('selected','skipped') OR (routing_state='skipped' AND (state<>'cancelled' OR safe_code<>'WORKFLOW_OK'))").get() as Row, 'count'); const malformedAttempts = number(this.db().prepare("SELECT count(*) AS count FROM workflow_node_attempts WHERE external_attempt_id<>'' AND length(external_attempt_id)<>36").get() as Row, 'count'); const malformedWaits = number(this.db().prepare("SELECT count(*) AS count FROM workflow_approval_waits w LEFT JOIN workflow_runs r ON r.run_id=w.run_id LEFT JOIN workflow_node_attempts a ON a.run_id=w.run_id AND a.node_id=w.node_id WHERE r.run_id IS NULL OR a.run_id IS NULL OR (w.phase IN ('waiting','reviewed') AND (r.state<>'stopping' OR a.state<>'ready')) OR (w.phase='waiting' AND (w.session_id<>'' OR w.challenge_digest<>'' OR w.receipt_digest<>'' OR w.expires_at<>'')) OR (w.phase='reviewed' AND (length(w.session_id)<>36 OR length(w.challenge_digest)<>64 OR w.challenge_digest GLOB '*[^0-9a-f]*' OR w.receipt_digest<>'' OR w.expires_at='')) OR (w.phase='approved' AND (length(w.session_id)<>36 OR length(w.challenge_digest)<>64 OR w.challenge_digest GLOB '*[^0-9a-f]*' OR length(w.receipt_digest)<>64 OR w.receipt_digest GLOB '*[^0-9a-f]*'))").get() as Row, 'count'); const malformedAdmissions = number(this.db().prepare("SELECT count(*) AS count FROM workflow_runs WHERE residual_process_count<0 OR NOT ((task_admission_id='' AND task_admission_digest='' AND task_id='' AND repository_id='') OR (length(task_admission_id)=36 AND length(task_admission_digest)=64 AND length(task_id)=36 AND length(repository_id)=36))").get() as Row, 'count'); return mismatchedRuns === 0 && malformedOutbox === 0 && malformedRouting === 0 && malformedAttempts === 0 && malformedWaits === 0 && malformedAdmissions === 0;
  }
