...
export class WorkflowRegistry implements KoggWorkflowService, BackendApplicationContribution {
  ...
  async onStart(): Promise<void> {
    try {
      ...
      this.migrate(); this.assertIntegrity(); const stored = await this.diagnostics(); if (stored.canonicalMismatchCount || stored.planMismatchCount) throw new WorkflowValidationError('WORKFLOW_STORE_INTEGRITY'); await fs.chmod(this.databasePath, 0o600).catch(error => { if (process.platform !== 'win32') throw error; });
  ...
  async saveVersion(input: { requestId: string; templateId: string; expectedVersionNumber: number; graph: unknown }): Promise<WorkflowMutationResult> {
    ...
    try {
      ...
      const result = this.transaction(input.requestId, requestDigest, () => {
        ...
        this.db().prepare('INSERT INTO template_versions(version_id,template_id,version_number,graph_digest,graph_json,created_at) VALUES(?,?,?,?,?,?)').run(versionId, input.templateId, versionNumber, graphDigest, canonicalJson(graph), createdAt);
        return { kind: 'completed', code: 'WORKFLOW_OK', version: { templateId: input.templateId, versionId, versionNumber, graphDigest, createdAt } } as const;
      });
  ...
  async compile(input: { requestId: string; versionId: string }): Promise<WorkflowMutationResult> {
    ...
    try {
      uuid(input.requestId); uuid(input.versionId); const row = this.version(input.versionId); const graph = this.compiler.decodeAndValidate(JSON.parse(text(row, 'graph_json')) as unknown); const requestDigest = workflowDigest('compiled-plan', input);
      const result = this.transaction(input.requestId, requestDigest, () => {
        ...
        this.db().prepare('INSERT INTO compiled_plans(plan_id,version_id,plan_digest,graph_digest,trust_spine_digest,editable_node_count,injected_anchor_count) VALUES(?,?,?,?,?,?,?)').run(projection.planId, projection.versionId, projection.planDigest, projection.graphDigest, projection.trustSpineDigest, projection.editableNodeCount, projection.injectedAnchorCount);
  ...
  async diagnostics(): Promise<{ integrity: boolean; foreignKeys: boolean; immutableTriggers: boolean; canonicalMismatchCount: number; planMismatchCount: number; versionCount: number; planCount: number; activeProcessCount: number; residualProcessCount: number; recoveryBacklogCount: number; sourceMapsPresent: boolean }> {
    ...
    let planMismatchCount = 0; for (const row of db.prepare('SELECT * FROM compiled_plans').all() as Row[]) { const version = this.version(text(row, 'version_id')); const graph = this.compiler.decodeAndValidate(JSON.parse(text(version, 'graph_json')) as unknown); const trustSpineDigest = workflowDigest('trust-spine', { schemaVersion: '1', anchors: ['anchor.spec-frozen','anchor.spec-approved','anchor.producer-separated','anchor.checks-complete','anchor.evidence-admitted','anchor.ranex-pass-current','anchor.merge-preflight','anchor.controlled-merge','anchor.cleanup-complete'] }); const expectedPlanDigest = workflowDigest('compiled-plan', { schemaVersion: '1', versionId: text(row, 'version_id'), graphDigest: text(version, 'graph_digest'), trustSpineDigest, editableNodeIds: graph.nodes.map(node => node.nodeId), anchors: ['anchor.spec-frozen','anchor.spec-approved','anchor.producer-separated','anchor.checks-complete','anchor.evidence-admitted','anchor.ranex-pass-current','anchor.merge-preflight','anchor.controlled-merge','anchor.cleanup-complete'] }); if (text(row, 'graph_digest') !== text(version, 'graph_digest') || text(row, 'trust_spine_digest') !== trustSpineDigest || text(row, 'plan_digest') !== expectedPlanDigest) planMismatchCount++; }
    const packageRuntime = path.basename(__filename) === 'workflow-registry.js'; const sourceMapsPresent = existsSync(`${__filename}.map`) && (!packageRuntime || existsSync(path.join(__dirname, 'workflow-compiler.js.map')));
    return { integrity, foreignKeys, immutableTriggers, canonicalMismatchCount, planMismatchCount, versionCount: this.versionCount(), planCount: number(db.prepare('SELECT count(*) AS count FROM compiled_plans').get() as Row, 'count'), activeProcessCount: 0, residualProcessCount: 0, recoveryBacklogCount: 0, sourceMapsPresent };
  }
  ...
  private migrate(): void { this.db().exec(`CREATE TABLE IF NOT EXISTS template_versions(version_id TEXT PRIMARY KEY,template_id TEXT NOT NULL,version_number INTEGER NOT NULL CHECK(version_number>0),graph_digest TEXT NOT NULL,graph_json TEXT NOT NULL,created_at TEXT NOT NULL,UNIQUE(template_id,version_number));
    CREATE TABLE IF NOT EXISTS compiled_plans(plan_id TEXT PRIMARY KEY,version_id TEXT NOT NULL UNIQUE REFERENCES template_versions(version_id),plan_digest TEXT NOT NULL UNIQUE,graph_digest TEXT NOT NULL,trust_spine_digest TEXT NOT NULL,editable_node_count INTEGER NOT NULL,injected_anchor_count INTEGER NOT NULL);
...
function versionProjection(row: Row): WorkflowTemplateVersionProjection { return { templateId: text(row, 'template_id'), versionId: text(row, 'version_id'), versionNumber: number(row, 'version_number'), graphDigest: text(row, 'graph_digest'), createdAt: text(row, 'created_at') }; }
function plan(row: Row) { return { planId: text(row, 'plan_id'), versionId: text(row, 'version_id'), planDigest: text(row, 'plan_digest'), graphDigest: text(row, 'graph_digest'), trustSpineDigest: text(row, 'trust_spine_digest'), editableNodeCount: number(row, 'editable_node_count'), injectedAnchorCount: number(row, 'injected_anchor_count') }; }
