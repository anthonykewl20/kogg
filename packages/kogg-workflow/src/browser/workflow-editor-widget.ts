...
export class WorkflowEditorWidget extends BaseWidget {
  ...
  private versions: readonly WorkflowTemplateVersionProjection[] = []; private status = 'Loading workflow editor…'; private busy = false; private view: 'outline' | 'canvas' = 'outline';
  ...
  private render(): void {
    ...
    const canvas = this.nodes.map((item, index) => `<article role="listitem" tabindex="0" data-workflow-canvas-node="${html(item.nodeId)}"><span>Step ${index + 1}</span><strong>${html(item.kind)}</strong><p>${item.requestedEffects.length} bounded effects · retry ${item.retry.maxAttempts}</p><div class="kogg-package-actions"><button data-canvas-up="${index}" ${this.busy || index === 0 ? 'disabled' : ''} aria-label="Move ${html(item.kind)} earlier on canvas">Earlier</button><button data-canvas-down="${index}" ${this.busy || index === this.nodes.length - 1 ? 'disabled' : ''} aria-label="Move ${html(item.kind)} later on canvas">Later</button><button data-canvas-remove="${index}" ${this.busy || this.nodes.length === 1 ? 'disabled' : ''} aria-label="Remove ${html(item.kind)} from canvas">Remove</button></div></article>`).join('');
  ...
  private bindEvents(): void {
    ...
    this.node.querySelectorAll<HTMLElement>('[data-canvas-remove]').forEach(button => button.addEventListener('click', () => { this.nodes.splice(Number(button.dataset.canvasRemove), 1); this.resetEdges(); this.status = 'Node removed.'; this.render(); }));
