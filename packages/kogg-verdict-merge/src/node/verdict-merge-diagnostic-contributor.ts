...
function check(id: typeof VERDICT_MERGE_CHECKS[number], value: ReturnType<VerdictMergeService['diagnostics']>, authorization?: ReturnType<MergeAuthorizationRegistry['diagnostics']>, nativeGit?: ReturnType<NativeGitMergeService['diagnostics']>): boolean { if (id === 'merge.authorization') return authorization?.authorizationReady === true; if (id === 'merge.preflight') return authorization?.preflightReady === true; if (id === 'merge.atomicity') return authorization?.atomicityReady === true; if (id === 'merge.processes') return value.processCount === 0 && value.residualProcessCount === 0 && (nativeGit?.activeCount ?? 0) === 0; if (id === 'merge.source-maps') return value.sourceMapsPresent && authorization?.sourceMapsPresent === true && (nativeGit?.sourceMapsPresent ?? true); if (id === 'merge.recovery') return value.recoveryReady && authorization?.recoveryReady === true; const key = ({ 'verdict.provenance':'provenanceReady','verdict.bindings':'bindingsReady','verdict.currentness':'currentnessReady','verdict.explanation':'explanationReady' } as const)[id]; return key ? value[key] : false; }
