...
test('admits human merge authority only through authenticated same-origin CSRF-protected HTTP', { timeout: 20_000 }, async context => {
  ...
  const verdicts = new VerdictMergeService(new PassingAuthority(), MODE_AUTHORITY, path.join(root, 'verdict.sqlite3')); const authority = new MergeAuthorizationAuthority(); const registry = new MergeAuthorizationRegistry(verdicts, authority, MODE_AUTHORITY, path.join(root, 'authorization.sqlite3'), () => new Date('2026-08-27T00:00:10.000Z'));
  ...
  const authorized = await mutation(`${base}/kogg/merge/authorization/authorize`, authorizeBody, cookie, base, csrf); assert.equal(authorized.status, 200); const authorizedResult = await authorized.json() as { kind?: string; authorization?: { authorizationId?: string } }; assert.equal(authorizedResult.kind, 'authorized'); assert(authorizedResult.authorization?.authorizationId);
  const executeBody = { requestId: '60000000-0000-4000-8000-000000000003', authorizationId: authorizedResult.authorization.authorizationId };
  assert.equal((await mutation(`${base}/kogg/merge/authorization/execute`, executeBody, cookie, base, 'wrong')).status, 403);
  const accepted = await mutation(`${base}/kogg/merge/authorization/execute`, executeBody, cookie, base, csrf); assert.equal(accepted.status, 202); assert.equal((await accepted.json() as { kind?: string }).kind, 'accepted');
});
