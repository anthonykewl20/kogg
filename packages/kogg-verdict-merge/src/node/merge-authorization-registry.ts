...
export class MergeAuthorizationRegistry implements BackendApplicationContribution {
  ...
  private verifyIntegrity(): void {
    if (text(this.db().prepare('PRAGMA quick_check').get() as Row, 'quick_check') !== 'ok' || this.countTriggers() !== 18) throw new Error('integrity');
