...
type IntentRecord = MergeIntentProjectionV1 & { readonly requestId: string; readonly authorizationDigest: string; readonly exactBindingsDigest: string; readonly repositoryIdentityDigest: string; readonly taskId: string; readonly taskRevisionId: string; readonly generation: '1' };
...
export class MergeAuthorizationRegistry implements BackendApplicationContribution {
  ...
  private verifyIntegrity(): void {
    if (text(this.db().prepare('PRAGMA quick_check').get() as Row, 'quick_check') !== 'ok' || this.countTriggers() !== 16) throw new Error('integrity');
...
function decodeIntentRecord(input: unknown): IntentRecord { const value = closed(input, ['mergeId','requestId','authorizationId','authorizationDigest','exactBindingsDigest','repositoryIdentityDigest','taskId','taskRevisionId','destinationRef','expectedOldOid','subjectOid','expectedTreeOid','mergePolicyId','state','generation','createdAt']); if (!UUID.test(String(value.mergeId)) || !UUID.test(String(value.requestId)) || !UUID.test(String(value.authorizationId)) || value.state !== 'preflight-pending' || value.generation !== '1') throw new Error('intent'); return value as unknown as IntentRecord; }
