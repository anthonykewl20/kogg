...
export function verdictMergeDigest(domain: 'query' | 'explanation' | 'challenge' | 'authorization' | 'intent', value: unknown): string { return createHash('sha256').update(`kogg:verdict-merge:${domain}:v1\n`).update(canonicalJson(value)).digest('hex'); }
