...
import { operationsSourceMapDiagnostics } from './operations-source-map-diagnostics';
...
export class OperationsReadModelDiagnosticContributor implements KoggDiagnosticContributor {
  ...
  async diagnose(): Promise<readonly KoggDiagnosticCheck[]> {
    try {
      ...
      const sourceMaps = operationsSourceMapDiagnostics(); const sourceMapsPresent = sourceMaps.missingCount === 0;
      return [
        ...
        { id: 'operations.source-maps', status: sourceMapsPresent ? 'pass' : 'fail', summary: sourceMapsPresent ? 'Every operations UI, protocol, projection, action, support, and process boundary has a debugger source map.' : 'One or more operations source maps are unavailable.', details: { ...sourceMaps } }
      ];
    } catch (error) {
