...
import type { CandidateBindingV1, CandidateImportIntentV1, FailCandidateImportV1, ImportedCandidateV1, RecordSealedCandidateV1 } from '../common/execution-protocol';
...
test('orders durable seal and import commits around the external candidate boundaries', async () => {
  ...
  const controller = new CandidateLifecycleController({
    ...
    completeCandidateImport: async () => { calls.push('import-record'); return imported; }, failCandidateImport: async () => { calls.push('import-quarantine'); return {} as never; }
...
test('never repeats an external import for a replayed ambiguous intent', async () => {
  let imports = 0; let quarantines = 0; const candidate = sealed(); const controller = new CandidateLifecycleController({ prepareCandidateImport: async () => intent(true), failCandidateImport: async () => { quarantines++; return {} as never; } } as never, {} as never, { import: async () => { imports++; throw new Error('must not run'); } });
  await assert.rejects(() => controller.import(importRequest(candidate)), (error: unknown) => error instanceof ImportError && error.code === 'IMPORT_FAILED'); assert.equal(imports, 0); assert.equal(quarantines, 1);
});

test('durably quarantines an import failure after intent before returning the refusal', async () => {
  const calls: string[] = []; const candidate = sealed(); const controller = new CandidateLifecycleController({
    prepareCandidateImport: async () => { calls.push('intent-record'); return intent(false); },
    failCandidateImport: async (request: FailCandidateImportV1) => { calls.push(`quarantine:${request.safeCode}`); return {} as never; }
  } as never, {} as never, { import: async () => { calls.push('import-cas'); throw new ImportError('IMPORT_SOURCE_INTEGRITY_FAILED'); } });
  await assert.rejects(() => controller.import(importRequest(candidate)), (error: unknown) => error instanceof ImportError && error.code === 'IMPORT_SOURCE_INTEGRITY_FAILED');
  assert.deepEqual(calls, ['intent-record', 'import-cas', 'quarantine:IMPORT_SOURCE_INTEGRITY_FAILED']);
});

test('fails closed when durable import quarantine cannot be recorded', async () => {
  const candidate = sealed(); const controller = new CandidateLifecycleController({
    prepareCandidateImport: async () => intent(false), failCandidateImport: async () => { throw new Error('registry unavailable'); }
  } as never, {} as never, { import: async () => { throw new ImportError('IMPORT_SOURCE_CHANGED'); } });
  await assert.rejects(() => controller.import(importRequest(candidate)), (error: unknown) => error instanceof ImportError && error.code === 'IMPORT_FAILED');
});
...
function importRequest(candidate: CandidateBindingV1): GovernedImportRequest { return { intentRequestId: '10000000-0000-4000-8000-000000000008', completionRequestId: '10000000-0000-4000-8000-000000000009', failureRequestId: '10000000-0000-4000-8000-00000000000b', expectedRevision: '9', bindingDigest: `sha256:${'5'.repeat(64)}`, expectedSourceIdentityDigest: `sha256:${'6'.repeat(64)}`, candidateImport: { projectId: '10000000-0000-4000-8000-000000000007', repositoryId: '10000000-0000-4000-8000-00000000000a', sourceRoot: '/source', sourceGitDirectory: '/source/.git', privateRoot: '/allocation/private', bundlePath: '/allocation/candidate.bundle', expectedSourceHead: 'a'.repeat(40), expectedSourceTree: 'b'.repeat(40), objectFormat: 'sha1', candidate } }; }
