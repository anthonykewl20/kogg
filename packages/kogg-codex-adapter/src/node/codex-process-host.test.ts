...
test('refuses registration before spawn and cleans a failed or unverified start', async () => {
  ...
  const timeout = harness({ spawnStalls: true }); await assert.rejects(timeout.register().start(), error => error instanceof CodexProcessHostFault && error.code === 'CODEX_SPAWN_TIMEOUT'); assert.equal(timeout.events.includes('process.cleaned'), true);
});
