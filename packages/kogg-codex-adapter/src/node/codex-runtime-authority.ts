...
export interface QualifiedCodexRuntimeAuthority {
  ...
  create(input: Omit<Parameters<AgentAdapterFactory['create']>[0], 'binding' | 'credentialLease' | 'operation'> & { readonly runtime: QualifiedCodexRuntimeV1; readonly attempt: GovernedCodexAttemptV1; readonly credentials: CodexCredentialReservation; readonly processes: Pick<CodexProcessHostGate, 'register'> }): AgentAdapterSession;
}
...
function lifecycleBound(session: AgentAdapterSession, credentials: CodexCredentialReservation, processes: CodexProcessHostGate): AgentAdapterSession {
  const settle = async (owner: () => Promise<unknown>): Promise<unknown> => { let ownerFailure: unknown; let result: unknown; let residualCount = 0; try { result = await owner(); } catch (error) { /* observability-exempt: The qualified owner must emit its closed failure; mandatory process and credential cleanup still runs. */ ownerFailure = error; } try { ({ residualCount } = await processes.cleanup()); } catch (error) { /* observability-exempt: Process cleanup emitted its closed failure; an earlier owner failure remains primary. */ if (!ownerFailure) ownerFailure = error; } try { await credentials.revoke(); } catch (error) { /* observability-exempt: Credential revocation already emitted its closed failure; an earlier lifecycle failure remains primary. */ if (!ownerFailure) ownerFailure = error; } if (ownerFailure) throw ownerFailure; if (result && typeof result === 'object' && 'residualCount' in result) return { residualCount: Math.max(Number((result as { residualCount: number }).residualCount), residualCount) }; return result; };
