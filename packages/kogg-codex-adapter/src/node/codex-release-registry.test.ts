...
function acceptedMethods(): Buffer { return Buffer.from(`${JSON.stringify({ schemaVersion: '1', protocolId: 'codex.app-server-v2', protocolVersion: '1.0.0', outboundRequests: ['initialize', 'thread/start', 'turn/start', 'turn/interrupt', 'shutdown'], outboundNotifications: ['initialized'], responses: [{ requestMethod: 'initialize', schemaDefinition: 'InitializeResponse' }, { requestMethod: 'thread/start', schemaDefinition: 'ThreadStartResponse' }, { requestMethod: 'turn/start', schemaDefinition: 'TurnStartResponse' }, { requestMethod: 'turn/interrupt', schemaDefinition: 'TurnInterruptResponse' }, { requestMethod: 'shutdown', schemaDefinition: 'ShutdownResponse' }], inbound: [{ method: 'item/completed', kind: 'notification', lifecycle: 'activity', content: 'routed', schemaDefinition: 'ItemCompletedNotification' }, { method: 'item/requestApproval', kind: 'server-request', lifecycle: 'authority-request', content: 'none', schemaDefinition: 'RequestApprovalParams' }, { method: 'turn/completed', kind: 'notification', lifecycle: 'turn-completed', content: 'none', schemaDefinition: 'TurnCompletedNotification' }, { method: 'turn/started', kind: 'notification', lifecycle: 'turn-started', content: 'none', schemaDefinition: 'TurnStartedNotification' }] })}\n`); }
function schemaBundle(): Buffer { const definitions = Object.fromEntries(['InitializeResponse', 'ThreadStartResponse', 'TurnStartResponse', 'TurnInterruptResponse', 'ShutdownResponse', 'ItemCompletedNotification', 'RequestApprovalParams', 'TurnCompletedNotification', 'TurnStartedNotification'].map(name => [name, { type: 'object' }])); return Buffer.from(`${JSON.stringify({ $schema: 'http://json-schema.org/draft-07/schema#', definitions, title: 'CodexAppServerProtocolV2', type: 'object' })}\n`); }
