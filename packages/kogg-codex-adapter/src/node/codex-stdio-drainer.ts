...
import { CodexClientFault } from './codex-protocol-client';
...
export class CodexStdioDrainer {
  ...
  constructor(private readonly protocol: CodexStdoutConsumer, private readonly onFault: (code: StdioFault) => void | Promise<void>, private readonly stderrLimit = MAX_STDERR_BYTES) { if (!Number.isSafeInteger(stderrLimit) || stderrLimit < 1 || stderrLimit > MAX_STDERR_BYTES) throw new Error('Invalid stderr limit'); }
  ...
  private async stdout(stream: Readable): Promise<void> { try { for await (const chunk of stream) if (!this.failed) try { await this.protocol.push(Buffer.from(chunk as Uint8Array)); } catch (error) { const known = error instanceof CodexProtocolFault || error instanceof CodexClientFault; await this.fail(known ? error.code as StdioFault : 'CODEX_TRANSPORT_LOST', known); } if (!this.failed) this.protocol.end(); } catch (error) { // observability-exempt: fail emits a closed code while raw stream errors and protocol bytes are discarded.
      const known = error instanceof CodexProtocolFault || error instanceof CodexClientFault; await this.fail(known ? error.code as StdioFault : 'CODEX_TRANSPORT_LOST', known); } }
