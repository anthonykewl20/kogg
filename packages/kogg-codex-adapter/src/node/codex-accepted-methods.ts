...
const ROOT_FIELDS = ['schemaVersion', 'protocolId', 'protocolVersion', 'outboundRequests', 'outboundNotifications', 'responses', 'inbound'] as const;
...
export interface AcceptedCodexMethods { readonly inbound: readonly AcceptedCodexMethod[]; readonly responses: readonly AcceptedCodexResponse[]; readonly inboundSet: { readonly size: number; has(method: string): boolean }; }
...
export function parseAcceptedCodexMethods(bytes: Buffer): AcceptedCodexMethods {
  ...
  if (!record(raw) || !exact(raw, ROOT_FIELDS) || raw.schemaVersion !== '1' || raw.protocolId !== 'codex.app-server-v2' || raw.protocolVersion !== '1.0.0' || !same(raw.outboundRequests, REQUESTS) || !same(raw.outboundNotifications, NOTIFICATIONS) || !Array.isArray(raw.responses) || !Array.isArray(raw.inbound) || raw.responses.length !== REQUESTS.length || raw.inbound.length < 3 || raw.inbound.length > 256) invalid();
  ...
  return Object.freeze({ inbound: Object.freeze(inbound.map(value => Object.freeze(value))), responses: Object.freeze(responses.map(value => Object.freeze(value))), inboundSet });
}
export function validateCodexSchemaBundle(bytes: Buffer, accepted: AcceptedCodexMethods): void {
  ...
  const required = [...accepted.responses.map(value => value.schemaDefinition), ...accepted.inbound.map(value => value.schemaDefinition)];
