...
function fixture(sendTurnStarted = true, privatePrompt: unknown = true) {
...
test('composes verified host stdio through initialize, thread, turn, terminal cleanup, and empty enumeration', async () => {
  const value = fixture(); await value.session.start(); assert.deepEqual(value.observations, ['turn-started']); value.stdout.write(`${JSON.stringify({ method: 'turn/completed', lifecycle: 'turn-completed' })}\n`); await new Promise(resolve => setImmediate(resolve));
