...
function fixture(sendTurnStarted = true, privatePrompt: unknown = true, activationFails = false, observedModelId = 'gpt-5', stalledMethod?: string) {
  ...
  stdin.on('data', chunk => { buffer += Buffer.from(chunk).toString('utf8'); while (buffer.includes('\n')) { const end = buffer.indexOf('\n'); const frame = JSON.parse(buffer.slice(0, end)) as { id?: number; method?: string; error?: unknown }; buffer = buffer.slice(end + 1); if (frame.error && frame.id) { events.push(`authority.denied.${frame.id}`); continue; } if (frame.id && frame.method && frame.method !== stalledMethod) stdout.write(`${JSON.stringify({ id: frame.id, outcome: 'result', result: resultFor(frame.method, observedModelId) })}\n`); if (frame.method === 'turn/start' && sendTurnStarted) stdout.write(`${JSON.stringify({ method: 'turn/started', lifecycle: 'turn-started' })}\n`); events.push(`request.${frame.method}`); } });
  let streamsClosed = false; const closeStreams = (): void => { if (streamsClosed) return; streamsClosed = true; stdout.end(); stderr.end(); };
  const host: CodexLiveSessionHost = { processId: 'process-1', start: async () => { events.push('credentials.activated.process-1'); if (activationFails) throw new CodexCredentialFault('CODEX_CREDENTIAL_LEASE_REFUSED'); events.push('host.started'); return { stdin, stdout, stderr }; }, revokeCredentials: async () => { events.push('credentials.revoked'); }, terminateOwnedHost: async () => { events.push('host.terminated'); closeStreams(); }, enumerateResiduals: async () => { events.push('host.enumerated'); return 0; } };
  let contentClosed = 0; const session = new CodexLiveSession({ attemptId: 'attempt-1', operationId: 'operation-1', host, initializeParams: {}, threadParams: { ephemeral: true }, turnParams: { privatePrompt }, attestation: new CodexSessionAttestation({ attemptId: 'attempt-1', providerId: 'openai', modelId: 'gpt-5', capabilityIds: ['provider-turn'], mapper: { initialize: value => value, thread: value => value } }), onObservation: (_sequence, observation) => { if (observation.kind === 'notification') observations.push(observation.lifecycle); }, onFault: code => { faults.push(code); }, initializeTimeoutMs: 10, threadTimeoutMs: 10, startTimeoutMs: 20, interruptTimeoutMs: 10, cleanupTimeoutMs: 20,
...
test('composes verified host stdio through initialize, thread, turn, terminal cleanup, and empty enumeration', async () => {
  ...
  assert.deepEqual(await value.session.cleanup(), { terminalCode: 'CODEX_OK', residualCount: 0, cleaned: true }); assert.equal(value.events.indexOf('credentials.revoked') < value.events.indexOf('request.shutdown'), true); assert.equal(value.events.filter(event => event === 'credentials.activated.process-1').length, 1); assert.equal(value.events.filter(event => event === 'credentials.revoked').length, 1); assert.deepEqual(value.events.slice(-2), ['host.terminated', 'host.enumerated']); assert.equal(value.contentClosed() >= 1, true); assert.deepEqual(value.faults, []);
});
...
test('an authority request is denied before the live attempt fails and owned cleanup proves zero residuals', async () => {
  const value = fixture(); await value.session.start(); value.stdout.write(`${JSON.stringify({ id: 91, method: 'item/approval', lifecycle: 'authority-request' })}\n`);
  for (let count = 0; count < 20 && !value.faults.length; count++) await new Promise(resolve => setImmediate(resolve));
...
test('initialize and thread stalls retain exact phase timeout codes and complete mandatory cleanup', async () => {
  const initialize = fixture(true, true, false, 'gpt-5', 'initialize'); await assert.rejects(initialize.session.start(), error => error instanceof CodexLiveSessionFault && error.code === 'CODEX_INITIALIZE_TIMEOUT' && error.cleanup.cleaned); assert.equal(initialize.events.includes('request.thread/start'), false); assert.deepEqual(initialize.events.slice(-2), ['host.terminated', 'host.enumerated']);
  const thread = fixture(true, true, false, 'gpt-5', 'thread/start'); await assert.rejects(thread.session.start(), error => error instanceof CodexLiveSessionFault && error.code === 'CODEX_THREAD_START_TIMEOUT' && error.cleanup.cleaned); assert.equal(thread.events.includes('request.turn/start'), false); assert.deepEqual(thread.events.slice(-2), ['host.terminated', 'host.enumerated']);
});
test('interrupt acknowledgement timeout escalates to owned termination while preserving cancellation after zero-residual proof', async () => {
  const logs: string[] = []; const original = console.warn; console.warn = (...values: unknown[]) => logs.push(JSON.stringify(values));
  try { const value = fixture(true, true, false, 'gpt-5', 'turn/interrupt'); await value.session.start(); assert.deepEqual(await value.session.cancel(), { terminalCode: 'CODEX_CANCELLED', residualCount: 0, cleaned: true }); assert.equal(value.events.filter(event => event === 'request.turn/interrupt').length, 1); assert.equal(value.events.indexOf('request.turn/interrupt') < value.events.indexOf('request.shutdown'), true); assert.deepEqual(value.events.slice(-2), ['host.terminated', 'host.enumerated']); assert.match(logs.join('\n'), /CODEX_INTERRUPT_TIMEOUT/u); }
