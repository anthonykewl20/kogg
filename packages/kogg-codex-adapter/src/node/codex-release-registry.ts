...
const ADAPTER_VERSION = '1.0.0'; const MAX_MANIFEST_BYTES = 64 * 1024; const MAX_VERSION_BYTES = 256;
...
export class CodexReleaseRegistry implements BackendApplicationContribution {
  ...
  private async verifyBundle(): Promise<void> {
    ...
    try {
      ...
      await exactAsset(binary, manifest.binarySha256, Number(manifest.binarySize), true, 'CODEX_BINARY_MISMATCH');
      await exactAsset(path.join(releaseRoot, 'app-server-schema-v2.json'), manifest.appServerSchemaSha256, undefined, false, 'CODEX_SCHEMA_MISMATCH');
      await exactAsset(path.join(releaseRoot, 'accepted-methods.json'), manifest.acceptedMethodsSha256, undefined, false, 'CODEX_SCHEMA_MISMATCH');
      await exactAsset(path.join(releaseRoot, 'linux-helper'), manifest.linuxHelperSha256, undefined, true, 'CODEX_BINARY_MISMATCH');
...
async function exactAsset(file: string, digest: string, size: number | undefined, executable: boolean, code: CodexSafeCode): Promise<void> { try { const stat = await lstat(file); if (!stat.isFile() || stat.isSymbolicLink() || !owned(stat.uid) || (stat.mode & 0o022) !== 0 || (executable && (stat.mode & 0o111) === 0) || (size !== undefined && stat.size !== size)) throw new Error(); const bytes = await readFile(file); if (createHash('sha256').update(bytes).digest('hex') !== digest) throw new Error(); } catch { throw new ReleaseError(code); } }
