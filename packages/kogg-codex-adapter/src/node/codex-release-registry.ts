...
// Windows is never a qualified runtime target; ignoring its synthetic POSIX mode bits keeps injected Linux classification tests deterministic without weakening the real Linux gate.
function secureMode(mode: number, executable: boolean): boolean { return process.platform === 'win32' || (mode & 0o022) === 0 && (!executable || (mode & 0o111) !== 0); }
