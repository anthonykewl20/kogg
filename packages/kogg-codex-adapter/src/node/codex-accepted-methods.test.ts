...
test('binds every accepted method and response to the exact closed draft-07 schema bundle', () => {
  const accepted = parse(valid()); const names = [...accepted.responses, ...accepted.inbound].map(value => value.schemaDefinition); const definitions = Object.fromEntries(names.map(name => [name, { type: 'object' }])); const bundle = (value: unknown) => Buffer.from(JSON.stringify(value));
