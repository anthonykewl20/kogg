...
export class HarnessRunManifest {
    ...
    async completed({ fixtures, artifacts = [], residualCount = 0, verification, productDiagnostics }) {
        const final = finalFields(fixtures, artifacts, residualCount, verification, productDiagnostics); if (this.value.scenarioState !== 'completed' || this.value.steps?.some(step => step.state !== 'passed') || final.productDiagnostics.coverage !== 'complete' || final.harnessChecks.some(check => check.status !== 'pass')) throw new Error('E2E_MANIFEST_SUCCESS_REFUSED');
        await this.#transition('completed', final); this.#log('residual-check.completed', { residualCount }); this.#log('run.completed');
    }
    async failed({ fixtures, artifacts = [], residualCount = 0, verification = { oracle: 'fail', sourceMap: 'fail', mappedCount: 0 }, productDiagnostics = { coverage: 'incomplete', checkCount: 0, passCount: 0, warnCount: 0, failCount: 0 }, safeCode = 'E2E_SCENARIO_FAILED', errorType = 'Error' }) {
        if (!SAFE.test(safeCode) || !['Error','AssertionError','TimeoutError'].includes(errorType)) throw new Error('E2E_MANIFEST_INVALID');
        const final = finalFields(fixtures, artifacts, residualCount, verification, productDiagnostics); if (safeCode === 'E2E_ARTIFACT_UNSAFE') final.harnessChecks.find(check => check.id === 'e2e.artifacts').status = 'fail'; if (safeCode === 'E2E_PROCESS_RESIDUAL') final.harnessChecks.find(check => check.id === 'e2e.processes').status = 'fail';
...
function finalFields(fixtures, artifacts, residualCount, verification, productDiagnostics) {
    ...
    if (!productDiagnostics || Object.keys(productDiagnostics).sort().join(',') !== 'checkCount,coverage,failCount,passCount,warnCount' || !['complete','incomplete'].includes(productDiagnostics.coverage) || ['checkCount','passCount','warnCount','failCount'].some(key => !Number.isSafeInteger(productDiagnostics[key]) || productDiagnostics[key] < 0) || productDiagnostics.checkCount !== productDiagnostics.passCount + productDiagnostics.warnCount + productDiagnostics.failCount || productDiagnostics.coverage === 'complete' && productDiagnostics.checkCount < 1) throw new Error('E2E_MANIFEST_INVALID');
    ...
    return { fixtures: validatedFixtures, artifacts: validatedArtifacts, productDiagnostics: { ...productDiagnostics }, residualCount, harnessChecks: [
