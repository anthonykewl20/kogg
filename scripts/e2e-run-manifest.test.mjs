...
test('atomically records the complete successful run lifecycle and closed checks', async () => {
    ...
    try { const run = await HarnessRunManifest.create({ root, runtime: 'browser', platform: 'linux', capabilities: platformCapabilities('browser', 'linux'), runId, logger: line => logs.push(line) }); await run.starting(); await run.active('portable-surface'); await run.scenarioCompleted(); await run.cleaning([{ ...cleaned[0], state: 'ready', exitClass: 'unknown' }]); await run.completed({ fixtures: cleaned, residualCount: 0 }); const value = await run.read(); assert.equal(value.state, 'completed'); assert.equal(value.scenarioState, 'completed'); assert.deepEqual(value.steps, [{ id: 'visible-journey', state: 'passed' }]); assert.deepEqual(value.capabilities.map(capability => capability.status), ['pending-qualification','available']); assert.deepEqual(value.harnessChecks.map(check => check.status), ['pass','pass','pass']); assert.deepEqual(logs.map(line => line.match(/(?:run|scenario|step|residual-check)\.[a-z.]+/u)?.[0]).filter(Boolean), ['run.requested','run.started','scenario.started','step.started','step.completed','scenario.completed','residual-check.started','residual-check.completed','run.completed']); }
...
test('cleanup failure overrides success and terminal states cannot be rewritten', async () => {
    ...
    try { const run = await HarnessRunManifest.create({ root, runtime: 'electron', platform: 'macos', capabilities: platformCapabilities('electron', 'macos'), runId }); await run.starting(); await run.active('portable-surface'); await run.scenarioFailed(); await run.cleaning(cleaned); const residual = [{ ...cleaned[0], state: 'residual', exitClass: 'unknown' }]; await assert.rejects(run.completed({ fixtures: residual, residualCount: 1 }), /E2E_MANIFEST_SUCCESS_REFUSED/u); assert.equal((await run.read()).state, 'cleaning'); await run.failed({ fixtures: residual, residualCount: 1, safeCode: 'E2E_PROCESS_RESIDUAL' }); const failed = await run.read(); assert.equal(failed.state, 'failed'); assert.equal(failed.scenarioState, 'failed'); assert.deepEqual(failed.steps, [{ id: 'visible-journey', state: 'failed', safeCode: 'E2E_STEP_FAILED' }]); assert.equal(failed.harnessChecks.find(check => check.id === 'e2e.processes').status, 'fail'); await assert.rejects(run.failed({ fixtures: residual, residualCount: 1 }), /E2E_MANIFEST_TRANSITION_INVALID/u); }
    finally { await rm(root, { recursive: true, force: true }); }
});
