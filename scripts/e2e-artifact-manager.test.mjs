...
test('retains only closed harness lifecycle facts and a safe atomic failure manifest', async () => {
    ...
    try {
        ...
        const lifecycle = await readFile(path.join(value.directory, 'lifecycle.log'), 'utf8'); const manifest = await readFile(path.join(value.directory, 'manifest.json'), 'utf8');
        assert.match(lifecycle, /fixture\.cleanup\.completed/u); assert.doesNotMatch(`${lifecycle}\n${manifest}`, /private|bearer|token=|source code/iu);
        assert.match(manifest, /E2E_ARTIFACT_CONTENT_BEARING/u); assert.deepEqual(logs.map(line => line.match(/artifact\.[a-z.]+/u)?.[0]), ['artifact.capture.started','artifact.capture.completed']);
