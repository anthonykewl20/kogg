...
test('retains only closed harness lifecycle facts and a safe atomic failure manifest', async () => {
    ...
    try {
        ...
        assert.match(lifecycle, /run\.started/u); assert.match(lifecycle, /discovery\.attempt.*"attempt":2/u); assert.match(lifecycle, /scenario\.failed/u); assert.match(lifecycle, /source-map\.completed.*"mappedCount":11/u); assert.match(lifecycle, /fixture\.cleanup\.completed/u); assert.doesNotMatch(`${lifecycle}\n${failure}`, /private|bearer|token=|source code/iu);
