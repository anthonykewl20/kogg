...
recover in the UI, admission return to enabled, diagnostics pass, a replacement
Ranex bridge become active, and every pre-crash process identity disappear. On
...
for qualified Linux. Before the production harness may claim crash recovery on
macOS or Windows, #70 must add a platform-native creation identity or a contained
lifetime that makes product descendants exit with the application. PID-only
signalling and converting blocked admission into a passing capability are both
rejected.
